...
export class MemoryStore {
  ...
  appendAction(action: string, args: unknown, promptExcerpt: string, modelRaw: string): void {
    this.db
      .query(
        "INSERT INTO actions (ts, action, args, prompt_excerpt, model_raw) VALUES (?, ?, ?, ?, ?)"
      )
      .run(nowIso(), action, toJson(args), truncate(promptExcerpt), truncate(modelRaw));
  }
  ...
  getRecentEvents(limit: number): StoredEvent[] {
    const rows = this.db
      .query("SELECT id, ts, type, payload FROM events ORDER BY id DESC LIMIT ?")
