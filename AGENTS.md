...
Note: Personality is now custom-generated by the LLM during registration (title + behavior description). The old `--personality` flag has been removed.

Examples:
```bash
bun run ollama-play -n alice
bun run ollama-play -n test-bot -e crimson -a evil -s gritty
```
...
### Run Swarm
```bash
bun run swarm [--count 5] [--prefix swarm] [--empire <empire>] [--alignment <alignment>] [--speech-style <style>]
```
...
Example:
```bash
bun run swarm --count 10 --empire crimson --alignment evil --speech-style gritty
```
